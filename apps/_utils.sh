...
utils:github_latest_commit_hash() {
    ...
    curl -A "Mozilla/5.0" "https://github.com/$repo/commits/$branch.atom" |
...
utils:github_latest_tag() {
    ...
    curl -A "Mozilla/5.0" "https://github.com/$repo/tags.atom" |
...
utils:github_releases() {
    ...
    curl -A "Mozilla/5.0" "https://github.com/$repo/releases.atom" |
...
utils:gitlab_latest_tag() {
    ...
    curl -A "Mozilla/5.0" "https://$gitlab_host/$repo/-/tags?format=atom" |
...
utils:cgit_lastest_tag() {
    ...
    curl -A "Mozilla/5.0" "$cgit_repo_url/refs/" |
...
utils:gitea_tags() {
    ...
    curl -A "Mozilla/5.0" "https://$gitea_host/$repo/tags" |
        sed -n 's%.*/src/tag/\([^"]\+\)"[ >].*%\1%p'
}
